...
declare type InvalidQueryFn = QueryFunction<undefined | Promise<undefined> | void | Promise<void>>;
...
declare type GetOptions<T> = T extends {
    ...
    queryFn?: InvalidQueryFn;
} ? never | 'queryFn must not return undefined or void' : T extends {
 ...
 */
export declare type QueriesOptions<T extends any[], Result extends any[] = [], Depth extends ReadonlyArray<number> = []> = Depth['length'] extends MAXIMUM_DEPTH ? UseQueryOptionsForUseQueries[] : T extends [] ? [] : T extends [infer Head] ? [...Result, GetOptions<Head>] : T extends [infer Head, ...infer Tail] ? QueriesOptions<[...Tail], [...Result, GetOptions<Head>], [...Depth, 1]> : unknown[] extends T ? T : T extends {
    queryFn: InvalidQueryFn;
}[] ? (never | 'queryFn must not return undefined or void')[] : T extends UseQueryOptionsForUseQueries<infer TQueryFnData, infer TError, infer TData, infer TQueryKey>[] ? UseQueryOptionsForUseQueries<TQueryFnData, TError, TData, TQueryKey>[] : UseQueryOptionsForUseQueries[];
/**
